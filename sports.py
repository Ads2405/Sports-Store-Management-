...
develop_label=Label(root, text="developed by -Anmol Deshratna Saxena  ", font=("Segoe Print", 18, "italic","bold"),
...
def submit(execute):
    ...
    if execute == 'del':
        ...
        cur.execute(f'delete from items_sold where i_code="{stockcode}"')
        myd.commit()
    ...
    if execute == "del1":
        ...
        cur.execute(f'delete from stock where i_code="{stockcode1}"')
        myd.commit()
    ...
    if execute == "update":   
        ...
        cur.execute(f'update items_sold set i_name="{name3}",i_code="{code3}",i_price="{price3}",quantity="{quantity3}",total_cost="{totalcost3}",customer_name="{customername3}",customer_no="{customerno3}",company="{company3}" where i_code="{code3}"')
        myd.commit()
...
lg=Label(Home,text="ENTER USERNAME :-",bg="#a8e063",fg="black",font=("Ariel Black",10))
lg.place(relx=.35,rely=.48)
lg2=Label(Home,text="ENTER PASSWORD :-",bg="#a8e063",fg="black",font=("Ariel Black",10))
...
image1=Image.open("e:/football.png")
...
image2=Image.open("e:/fs2.jpg")
image2=image2.resize((75,75),Image.ANTIALIAS)
my_img2=ImageTk.PhotoImage(image2)
lbl_2=Label(image=my_img2,bg="#d76d77").place(relx=.03,rely=.065)

image3=Image.open("e:/bs1.jpg")
image3=image3.resize((75,75),Image.ANTIALIAS)
my_img3=ImageTk.PhotoImage(image3)
lbl_3=Label(image=my_img3,bg="#d76d77").place(relx=.92,rely=.065)
...
btn_clear1= Button(add_rec_ITEM,text="CLEAR",padx=50,pady=15).place(relx=.3,rely=.9)
...
btn_submit6= Button(update_rec_STOCK,text="SUBMIT",padx=50,pady=15,command=lambda: submit('add1')).place(relx=.55,rely=.9)

btn_clear6= Button(update_rec_STOCK,text="CLEAR",padx=50,pady=15).place(relx=.3,rely=.9)
...
myd= mysql.connector.connect(host="localhost",user="root",passwd="anmol245", database= "anmolIP")
